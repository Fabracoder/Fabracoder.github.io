...
function interpolateByFour(a,b,c,d) {
    'use strict';
    return (interpolateNumbers(a,b,t1))+(interpolateNumbers(c,d,t2));
}
...
// Calculates the interpolated point for a 5 point bezier curve
function BezierFive(p1, p2, p3, p4, p5, t) {
    'use strict';
    var tempXa = (1 - t) * cubicBezier(p1, p2, p3,p4, t).x + t * (cubicBezier(p2, p3, p4, p5, t).x);
    var tempYa = (1 - t) * cubicBezier(p1, p2, p3,p4, t).y + t * (cubicBezier(p2, p3, p4, p5, t).y);
    
    return new Point(tempXa, tempYa);
}
...
function Line(p1, p2) {
    ...
    this.dotted = function (steps) {
        ...
        // This loop gets the interpolated Points and draws them
    ...
    this.dashed = function (steps) {
        ...
        // This loop gets the interpolated Points and connects every other one with a line
        for (temp = 0; temp < 1; temp = temp + ratio) {
            if (i === 0) {
                ...
                if(steps % 2 === 0) // trying to fix my issue with even steps
                    { if (temp + ratio <= 1) {
                        context.stroke();
                    }
                }else
...
// Calculates the interpolated point for a quadratic bezier curve -delete
...
Quadratic.prototype.dotted = function (steps) {
    ...
    var temp = 0;
    for (temp = 0; temp <= 1; temp += ratio) {
        quadraticBezier(this.p1, this.p2, this.p3, temp).draw();
    }
};
...
// a method for dotted Cubic curves
...
// 5 curve object constructor
function b5p(p1, p2, p3, p4,p5) {
    this.p1 = p1;
    this.p2 = p2;
    this.p3 = p3;
    this.p4 = p4;
    this.p5 = p5;
    dotted = function (steps) {
        'use strict';
        var temp = 0, ratio =  1.0 / steps;

        for (temp = 0; temp <= 1; temp += ratio) {
            BezierFive(this.p1, this.p2, this.p3, this.p4, this.p5, temp).draw();
        } 
    };
...
// a method for dotted 5 curves
b5p.prototype.dotted = function (steps) {
    'use strict';
    var temp = 0, ratio =  1.0 / steps;
